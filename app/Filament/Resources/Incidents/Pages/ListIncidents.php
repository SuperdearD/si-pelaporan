    ...
    public function getTabs(): array
    {
        return [
            ...
            'Disetujui' => Tab::make('Disetujui')
                ->modifyQueryUsing(fn(Builder $query) => $query->where('is_approved', true)->whereDoesntHave('followUps', fn($q) => $q->where('status_approval', 'Revisi')))
            ...
            'Direvisi' => Tab::make('Direvisi')
                ->modifyQueryUsing(fn(Builder $query) => $query->whereHas('followUps', fn($q) => $q->where('status_approval', 'Revisi')))
